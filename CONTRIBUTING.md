# Contributing to ImcodecNative

Thanks for helping improve ImcodecNative. Bug reports, focused fixes, documentation improvements, and well-scoped features are welcome.
...
- Search [existing issues](https://github.com/focale-editor/imcodec-native/issues) before opening a new one.
...
ImcodecNative follows the SDK constraint in [`pubspec.yaml`](pubspec.yaml). Install a compatible stable Flutter SDK, fork the repository, and run:

```sh
git clone https://github.com/YOUR_ACCOUNT/imcodec-native.git
cd imcodec-native
flutter pub get
```
...
Native and WebAssembly work has additional prerequisites and checks in [docs/BUILDING.md](docs/BUILDING.md).
